...
class LlamaAttention(nn.Module):
    ...
    def _load_qkvo(self, layer_idx=0):
        llama_ckpt_path = "/data/models/meta-llama/Llama-3.1-8B-Instruct/model-00001-of-00004.safetensors"
        q0_ckpt = load_file(llama_ckpt_path)["model.layers.0.self_attn.q_proj.weight"]
        k0_ckpt = load_file(llama_ckpt_path)["model.layers.0.self_attn.k_proj.weight"]
        v0_ckpt = load_file(llama_ckpt_path)["model.layers.0.self_attn.v_proj.weight"]
        o0_ckpt = load_file(llama_ckpt_path)["model.layers.0.self_attn.o_proj.weight"]
        self.q_proj.load_state_dict({"weight": q0_ckpt})
    ...
    def _init_rope(self):
        ...
        if self.config.rope_scaling is None:
            ...
            print("Llama rope scaling is none : LlamaRotaryEmbedding")
        else:
            try:
    ...
    def forward(
        ...
        if past_key_value is not None:
            ...
            intermediates["K_past"] = past_key_value[0]
            intermediates["V_past"] = past_key_value[1]
        ...
        if attention_mask is not None:
            ...
            intermediates["attn_weights_post_mask"] = attn_weights
        ...
        return attn_output, intermediates  # , attn_weights, past_key_value
...
if __name__ == "__main__":
    ...
    # Load reference data (Aayush collect)
    ...
    print("Layernorm checkpoint : ", rms_norm_ckpt.shape, rms_norm_ckpt.dtype)
    ...
    example["outputs/intermediates"] = intermediates
